...
sgx_status_t process_RA_status(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint8_t* attestationStatus, uint8_t* cmacStatus, uint8_t* p_payload, int payLen, int cryptLen, uint8_t* p_iv, uint8_t* sealedSecret)
{
	sgx_status_t status;
	ms_process_RA_status_t ms;
	ms.ms_context = context;
	ms.ms_attestationStatus = attestationStatus;
	ms.ms_cmacStatus = cmacStatus;
	ms.ms_p_payload = p_payload;
	ms.ms_payLen = payLen;
	ms.ms_cryptLen = cryptLen;
	ms.ms_p_iv = p_iv;
	ms.ms_sealedSecret = sealedSecret;
	status = sgx_ecall(eid, 1, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t unsealSecret(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* sealedSecret)
{
	sgx_status_t status;
	ms_unsealSecret_t ms;
	ms.ms_sealedSecret = sealedSecret;
...
sgx_status_t encryptWithSecretKey(sgx_enclave_id_t eid, sgx_status_t* retval, uint32_t length, uint8_t* clear, uint8_t* crypt, uint8_t* crypt_mac, uint8_t* iv)
{
	sgx_status_t status;
	ms_encryptWithSecretKey_t ms;
	ms.ms_length = length;
	ms.ms_clear = clear;
	ms.ms_crypt = crypt;
	ms.ms_crypt_mac = crypt_mac;
	ms.ms_iv = iv;
...
sgx_status_t decryptWithSecretKey(sgx_enclave_id_t eid, sgx_status_t* retval, uint32_t length, uint8_t* clear, uint8_t* crypt, uint8_t* crypt_mac, uint8_t* iv)
{
	sgx_status_t status;
	ms_decryptWithSecretKey_t ms;
	ms.ms_length = length;
	ms.ms_clear = clear;
	ms.ms_crypt = crypt;
	ms.ms_crypt_mac = crypt_mac;
