...
size_t memcpy_encrypt_or_seal(const void* plaintext_buffer, size_t plaintext_element_size, size_t plaintext_element_count, void* outbuffer,
                              uint32_t output_data_size, int (*encrypt_or_seal)(const void*, uint32_t, void*, uint32_t))

{
  ...
  // STEP 2
  // sgx lib outputs sealed data into enclave memory, so we have to copy this outside in the fwrite ocall
  check(memcpy_enclave_memory_ocall(&written_bytes, output_buffer, output_data_size, outbuffer, output_data_size));
  // free temp buffer (don't need to memset, encrypted anyway)
  free(output_buffer);
...
size_t memcpy_decrypt_or_unseal(void* plaintext_buffer, size_t plaintext_element_size, size_t plaintext_element_count, void* stream,
                                 uint32_t output_data_size, int (decrypt_or_unseal)(void*, uint32_t, void*))

{
  ...
  // temporary buffer (sealed_data) must be inside enclave, enforced by SGX lib, so we let the fread ocall copy it inside
  void* output_buffer = malloc(output_data_size);
  check(memcpy_memory_enclave_ocall(&read_bytes, output_buffer, output_data_size, stream, output_data_size));
...
size_t memcpy_encrypted(const void* plaintext_buffer, size_t plaintext_element_size, size_t plaintext_element_count, void* outbuffer)
{
	return memcpy_encrypt_or_seal(plaintext_buffer, plaintext_element_size, plaintext_element_count, outbuffer,
    get_encrypted_data_size(plaintext_element_count * plaintext_element_size), &encrypt_with_set_key);
}

size_t memcpy_decrypted(void* plaintext_buffer, size_t plaintext_element_size, size_t plaintext_element_count, void* outbuffer)
{
	return memcpy_decrypt_or_unseal(plaintext_buffer, plaintext_element_size, plaintext_element_count, outbuffer,
    get_encrypted_data_size(plaintext_element_count * plaintext_element_size), &decrypt_with_set_key);
}


size_t memcpy_sealed(const void* plaintext_buffer, size_t plaintext_element_size, size_t plaintext_element_count, void* outbuffer)
{
	return memcpy_encrypt_or_seal(plaintext_buffer, plaintext_element_size, plaintext_element_count, outbuffer,
    get_sealed_data_size(plaintext_element_count * plaintext_element_size), &seal_with_cast);
}

size_t memcpy_unsealed(void* plaintext_buffer, size_t plaintext_element_size, size_t plaintext_element_count, void* outbuffer)
{
	return memcpy_decrypt_or_unseal(plaintext_buffer, plaintext_element_size, plaintext_element_count, outbuffer,
