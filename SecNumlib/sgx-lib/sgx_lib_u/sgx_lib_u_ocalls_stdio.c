...
size_t memcpy_enclave_memory_ocall(const void* buffer, size_t size, void* stream, size_t outsize)
{
	return memcpy_s(stream, outsize, buffer, size);
}

size_t memcpy_memory_enclave_ocall( void* buffer, size_t size, void* stream, size_t outsize)
{
	return memcpy_s( buffer, size, stream, outsize);
}
